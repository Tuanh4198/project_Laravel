		...
		<div class="col-sm-9 padding-right" style="padding-top: 15px;">
			<div class="features_items">
				@foreach($brand_name as $key => $bn )
					<h2 class="title text-center">{{ $bn->brand_name }}</h2>
					<div class="introdute" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; border-bottom: 1px solid #f5f5f5;">
						<img src="{{URL::to('public/upload/brand/'.$bn->brand_img)}}" width="250" height="125">
						<p style="margin: 0px 15px 15px; font-style: italic; font-size: 15px;">{{ $bn->brand_desc }}</p>
					</div>
				@endforeach
				@foreach($list_product_brand_id as $key => $pro )
					<div class="col-sm-4">
